...
import { eq, and, sql, count, asc, isNotNull } from "drizzle-orm";
import type { DbExecutor, Transaction } from "../types.js";

type Round = typeof roundsTable.$inferSelect;
type PlayerSide = "host" | "guest";

export async function createRounds(data: (typeof roundsTable.$inferInsert)[], tx: Transaction) {
    return tx.insert(roundsTable).values(data).returning()
}
...
export async function getRoundByBattleId(battleId: string, executor: DbExecutor = db) {
    const res = await executor
        .select()
        .from(roundsTable)
        .where(eq(roundsTable.battleId, battleId))
        .orderBy(asc(roundsTable.roundNumber))
    return res
}

export async function getRoundsForUpdate(battleId: string, tx: Transaction) {
    return tx
        .select()
        .from(roundsTable)
        .where(eq(roundsTable.battleId, battleId))
...
export async function setRoundSong(
    roundId: string,
    side: PlayerSide,
    songId: string,
    tx: Transaction,
) {
    const patch = side === "host" ? { hostSongId: songId } : { guestSongId: songId };
    const [res] = await tx
        .update(roundsTable)
        .set(patch)
        .where(and(
            eq(roundsTable.id, roundId),
            eq(roundsTable.status, "WAITING"),
        )).returning()
    return res
}

export async function activateRound(roundId: string, tx: Transaction) {
    const [res] = await tx
        .update(roundsTable)
        .set({ status: "GUESSING" })
        .where(and(
            eq(roundsTable.id, roundId),
            eq(roundsTable.status, "WAITING"),
            isNotNull(roundsTable.hostSongId),
            isNotNull(roundsTable.guestSongId),
        ))
        .returning();
    return res;
}
